...
/* Replaces ghost become mouse behavior with spawn as snaprat instead */
...
/* Adding snaprats and ivenmoths to spawn criteria for desert and tundra */
	...
	switch(val)
		...
		if(6)
			if(prob(60))
				...
				new grass_path(T)
			if(prob(5))
				var/mob_type = pick(list(/mob/living/simple_animal/lizard, /mob/living/simple_animal/passive/mouse, /mob/living/simple_animal/passive/mouse/rat, /mob/living/simple_animal/passive/mouse/snaprat)) //ivenmoths shouldnt spawn in tundra, they're native to the desert. if eelocks or something get added, put them here
				new mob_type(T)
		if(7)
			if(prob(60))
				new /obj/structure/flora/bush(T)
...
/* Added snaprats and invemoths to infestation code */
#define VERM_MICE 0
