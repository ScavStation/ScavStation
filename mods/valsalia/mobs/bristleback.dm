...
/mob/living/simple_animal/hostile/bristleback
	...
	ai = /datum/mob_controller/aggressive/bristleback
	ability_handlers = list(/datum/ability_handler/predator)

/datum/mob_controller/aggressive/bristleback
...
/mob/living/simple_animal/hostile/bristleback/on_update_icon()
	. = ..()
	if(isspaceturf(loc))
...
/datum/mob_controller/aggressive/bristleback/find_target()
	. = ..()
	if(.)
		body.custom_emote(VISIBLE_MESSAGE,"stares alertly at [.]")
		set_stance(STANCE_ALERT)

/datum/mob_controller/aggressive/bristleback/do_process()
...
/mob/living/simple_animal/hostile/bristleback/attackby(var/obj/item/O, var/mob/user)
	if(istype(ai))
		var/stance = ai.get_stance()
		if(stance != STANCE_ATTACK && stance != STANCE_ATTACKING)
			ai.set_stance(STANCE_ALERT)
			if(istype(ai, /datum/mob_controller/aggressive/bristleback))
				var/datum/mob_controller/aggressive/bristleback/bristlebackbrain = ai
				bristlebackbrain.stance_step = 12
			ai.set_target(user)
	return ..()

/mob/living/simple_animal/hostile/bristleback/attack_hand(mob/user)
	if(istype(ai))
		var/stance = ai.get_stance()
		if(stance != STANCE_ATTACK && stance != STANCE_ATTACKING)
			ai.set_stance(STANCE_ALERT)
