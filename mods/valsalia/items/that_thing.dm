/obj/item/that_thing
	...
	icon_state = "thatthing"
	_base_attack_force = 1
	w_class = ITEM_SIZE_SMALL
	attack_verb = list("whipped")
	hitsound = 'sound/weapons/towelwhip.ogg'
	desc = "A mysterious stone artefact, careful if you're not human!"
	max_health = ITEM_HEALTH_NO_DAMAGE //un-fecking-breakable
	var/transform_impact_prob = 65
	var/transform_passive_prob = 10

/obj/item/that_thing/equipped()
	. = ..()
	if(!is_processing)
		START_PROCESSING(SSobj, src)

/obj/item/that_thing/dropped()
	..()
	if(is_processing)
		STOP_PROCESSING(SSobj, src)

/obj/item/that_thing/Destroy()
	if(is_processing)
		STOP_PROCESSING(SSobj, src)
	return ..()

/obj/item/that_thing/Process()
	if(!ishuman(loc))
		return PROCESS_KILL
	if(prob(transform_passive_prob))
		try_transform(loc)

/obj/item/that_thing/throw_impact(atom/hit_atom)
	..()
	if(ishuman(hit_atom) && prob(transform_impact_prob))
		try_transform(hit_atom)

/obj/item/that_thing/proc/try_transform(mob/living/human/victim)
	if(victim.species.name != SPECIES_HUMAN)
		victim.worn_underwear = null
		victim.change_species(SPECIES_HUMAN)
