/obj/item/flute
	name = "flute"
	desc = "A device used to emit rhythmic sounds, colloquialy refered to as a 'flute'. It's in a retro style (massive), and absoflutely unwieldy."
	icon = 'mods/valsalia/icons/flute.dmi'
...
/obj/item/flute/attack_self(var/mob/user)
	interact(user)

/obj/item/flute/Initialize()
	. = ..()
	sound_id = "[type]_[sequential_id(type)]"
	tracks = setup_music_tracks_fantasy_flute(tracks)

/obj/item/flute/examine(mob/user)
	. = ..()
	if(!panel)
...
/obj/item/flute/Destroy()
	stop()
	. = ..()

/obj/item/flute/interact(var/mob/user)
	...
	var/datum/browser/popup = new(user, "flute", "Flutists 1300", 290, 110)
	popup.set_content(dat)
	popup.open()

/obj/item/flute/DefaultTopicState()
	return global.physical_topic_state

/obj/item/flute/CouldUseTopic(var/mob/user)
	..()
	playsound(src, "switch", 40)

/obj/item/flute/OnTopic(var/user, var/list/href_list)
...
/obj/item/flute/attackby(var/obj/item/W, var/mob/user)
	if(IS_SCREWDRIVER(W))
		if(!panel)
...
/obj/item/flute/proc/AdjustFrequency(var/obj/item/W, var/mob/user)
...
/obj/item/flute/proc/MayAdjust(var/mob/user)
	if(playing)
		to_chat(user, "<span class='warning'>You can only adjust \the [src] when it's not playing.</span>")
		return FALSE
	return TRUE

/obj/item/flute/on_update_icon()
	. = ..()
	icon_state = get_world_inventory_state()
	if(playing)
		icon_state = "[icon_state]_on"

/obj/item/flute/proc/stop()
	playing = 0
	update_icon()
	QDEL_NULL(sound_token)

/obj/item/flute/proc/start()
	QDEL_NULL(sound_token)
...
/obj/item/flute/proc/change_volume(var/new_volume)
	volume = clamp(new_volume, 0, max_volume)
	if(sound_token)
		sound_token.SetVolume(volume)

/obj/random_multi/single_item/flute
	name = "flute spawnpoint"
	id = "boomtastic"
	item_path = /obj/item/flute

var/global/list/music_tracks_valsalia_fantasy_flutes = list(
	"Gaillarde Lesmerillone" = /decl/music_track/gaillarde_lesmerillone,
	"Gjendines Banlat" = /decl/music_track/gjendines_banlat,
	"Lullaby of Itsuki" = /decl/music_track/lullaby_of_itsuki,
	"Syrinx Debussy" = /decl/music_track/syrinx_debussy,
	"Traditional Flute 2" = /decl/music_track/traditional_flute2,
	"Traditional Nahua" = /decl/music_track/traditional_nahua,
	"Traditional Scythian Flute" = /decl/music_track/traditional_scythian_flute,
	"Vaishnava Jana To" = /decl/music_track/vaishnava_jana_to
	)

/proc/setup_music_tracks_fantasy_flute(var/list/tracks)
	. = list()
	var/track_list = LAZYLEN(tracks) ? tracks : global.music_tracks_valsalia_fantasy_flutes
