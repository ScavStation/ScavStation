/obj/item/drum
	name = "drum"
	desc = "A device used to emit rhythmic sounds, colloquialy refered to as a 'drum'. It's in a retro style (massive), and absodrumly unwieldy."
	icon = 'mods/valsalia/icons/drum.dmi'
...
/obj/item/drum/attack_self(var/mob/user)
	interact(user)

/obj/item/drum/Initialize()
	. = ..()
	sound_id = "[type]_[sequential_id(type)]"
	tracks = setup_music_tracks_fantasy_drum(tracks)

/obj/item/drum/examine(mob/user)
	. = ..()
	if(!panel)
...
/obj/item/drum/Destroy()
	stop()
	. = ..()

/obj/item/drum/interact(var/mob/user)
	...
	var/datum/browser/popup = new(user, "drum", "Drummy 1300", 290, 110)
	popup.set_content(dat)
	popup.open()

/obj/item/drum/DefaultTopicState()
	return global.physical_topic_state

/obj/item/drum/CouldUseTopic(var/mob/user)
	..()
	playsound(src, "switch", 40)

/obj/item/drum/OnTopic(var/user, var/list/href_list)
...
/obj/item/drum/attackby(var/obj/item/W, var/mob/user)
	if(IS_SCREWDRIVER(W))
		if(!panel)
...
/obj/item/drum/proc/AdjustFrequency(var/obj/item/W, var/mob/user)
...
/obj/item/drum/proc/MayAdjust(var/mob/user)
	if(playing)
		to_chat(user, "<span class='warning'>You can only adjust \the [src] when it's not playing.</span>")
		return FALSE
	return TRUE

/obj/item/drum/on_update_icon()
	. = ..()
	icon_state = get_world_inventory_state()
	if(playing)
		icon_state = "[icon_state]_on"

/obj/item/drum/proc/stop()
	playing = 0
	update_icon()
	QDEL_NULL(sound_token)

/obj/item/drum/proc/start()
	QDEL_NULL(sound_token)
...
/obj/item/drum/proc/change_volume(var/new_volume)
	volume = clamp(new_volume, 0, max_volume)
	if(sound_token)
		sound_token.SetVolume(volume)

/obj/random_multi/single_item/drum
	name = "drum spawnpoint"
	id = "boomtastic"
...
/proc/setup_music_tracks_fantasy_drum(var/list/tracks)
	. = list()
	var/track_list = LAZYLEN(tracks) ? tracks : global.music_tracks_valsalia_fantasy_drums
