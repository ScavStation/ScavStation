...
/datum/reagents/proc/remove_any(var/amount = 1, var/defer_update = FALSE) // Removes up to [amount] of reagents from [src]. Returns actual amount removed.
	...
	var/removing = FLOAT_FLOOR(Clamp(amount, 0, max(0, total_volume)), MINIMUM_CHEMICAL_VOLUME) // not ideal but something is making total_volume become NaN
	...
	if(part < MINIMUM_CHEMICAL_VOLUME)
		. = total_volume
		clear_reagents()
		return
	...
	. = 0
	while(removing >= MINIMUM_CHEMICAL_VOLUME)

		// Remove an even proportion from all reagents, if possible.
		var/all_too_low = TRUE
		for(var/current in reagent_volumes)
			var/removing_amt = FLOAT_FLOOR(REAGENT_VOLUME(src, current) * part, MINIMUM_CHEMICAL_VOLUME)
			if(removing_amt <= 0)
				continue
			all_too_low = FALSE
		...
		// We failed to remove anything, so nuke the list and return.
		if(all_too_low)
			. += total_volume
			clear_reagents()
			return
