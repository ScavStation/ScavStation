...
/mob/living/carbon/standard_weapon_hit_effects(obj/item/I, mob/living/user, var/effective_force, var/hit_zone)
	...
	if(istype(created_wound) && I && I.can_embed() && I.damtype == BRUTE && !I.anchored && !is_robot_module(I))
		...
		var/embed_chance = weapon_sharp? damage/NORMALIZE_ITEM_SIZE(I.w_class) : damage/(NORMALIZE_ITEM_SIZE(I.w_class)*3)
		var/embed_threshold = weapon_sharp? 5*NORMALIZE_ITEM_SIZE(I.w_class) : 15*NORMALIZE_ITEM_SIZE(I.w_class)

		//Sharp objects will always embed if they do enough damage.
		if((weapon_sharp && damage > (10*NORMALIZE_ITEM_SIZE(I.w_class))) || (damage > embed_threshold && prob(embed_chance)))
